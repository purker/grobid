...
public class CitationParser extends AbstractParser {
	...
	public BiblioItem processing(List<LayoutToken> tokens, boolean consolidate) {
		...
		try {
			...
			String ress = FeaturesVectorCitation.addFeaturesCitation(tokens, null, journalsPositions, abbrevJournalsPositions, conferencesPositions, publishersPositions, locationsPositions, collaborationsPositions, identifiersPositions, urlPositions);
			...
			if (resCitation != null) {
				...
				if (resCitation.getPublicationDate() != null) {
					List<Date> dates = parsers.getDateParser().processing(resCitation.getPublicationDate());
					if (dates != null) {
						Date bestDate = null;
						if (dates.size() > 0) {
	...
	public List<BibDataSet> processingReferenceSection(DocumentSource documentSource,
		...
		try {
			Document doc = parsers.getSegmentationParser().processing(documentSource, GrobidAnalysisConfig.builder().consolidateCitations(consolidate).build());
	...
	public StringBuilder trainingExtraction(List<String> inputs) {
		...
		try {
			...
			for (String input : inputs) {
				...
				String ress = FeaturesVectorCitation.addFeaturesCitation(tokenizations, null, journalsPositions, abbrevJournalsPositions, conferencesPositions, publishersPositions, locationsPositions, collaborationsPositions, identifiersPositions, urlPositions);
				String res = label(ress);
